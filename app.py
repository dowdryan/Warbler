...
def messages_show(message_id):
    ...
    if msg is None:
        # Return a 404 response if message is not found
...
def like_warble(message_id):
    """Like a specific warble."""
    if not g.user:
        flash("You do not have access", "danger")
        return redirect("/")
    message = Message.query.get_or_404(message_id)
    if not message.is_liked_by(g.user.id):
        Likes.like_warble(user_id=g.user.id, message_id=message.id)
    return redirect(url_for("homepage"))


@app.route('/users/remove_like/<int:message_id>/', methods=["POST"])
def unlike_warble(message_id):
    """Remove a like from a specific warble."""
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect("/")
    message = Message.query.get_or_404(message_id)
    if message.is_liked_by(g.user.id):
        Likes.unlike_warble(user_id=g.user.id, message_id=message.id)
        flash("Warble unliked successfully.", "success")
    else:
        flash("You haven't liked this warble.", "warning")
    return redirect(url_for("homepage"))


@app.route('/users/<int:user_id>/liked-warbles')
def liked_warble(user_id):
    """Route that displays the user's liked posts."""
    if not g.user:
        flash("You do not have access", "danger")
        return redirect("/")
    user = User.query.get_or_404(user_id)
