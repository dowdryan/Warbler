...
class Follows(db.Model):
    ...
    # Why are there 2 Foriegn Keys?
        # Represents Many-to-Many relationships between users.
...
class Likes(db.Model):
    ...
    message_id = db.Column(db.Integer,
                           ...
                           unique=True)
    @classmethod
    def like_warble(cls, user_id, message_id): 
        like = Likes(
            user_id=user_id,
        ...
        db.session.add(like)
        db.session.commit()
    def unlike_warble(cls, user_id, message_id):
...
class Message(db.Model):
    ...
    def is_liked_by(self, user_id):
        return Likes.query.filter_by(user_id=user_id, message_id=self.id).first() is not None
